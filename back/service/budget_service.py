...
def create_user_budget(db: sqlalchemy.orm.Session, data: budget_dto.BudgetCreate, user_id: int):
    category = db.query(structure.Category).filter(structure.Category.id_category == data.Categories_id_category).first()
    if not category:
        raise HTTPException(status_code=404, detail="Not found category for budget.")
    currency = db.query(structure.Currency).filter(structure.Currency.id_currency == data.Currency_id_currency).first()
    if not currency:
    ...
    return db.query(structure.BudgetAnalytics).filter(
        structure.BudgetAnalytics.id_budget == new_budget.id_budget
    ).first()

# USE THIS TO DISPLAY BUDGETS ON FRONTEND IT HAS ALL IMPORTANT DATA
def get_calculated_budgets(db: sqlalchemy.orm.Session, user_id: int) -> List[structure.BudgetAnalytics]:
    # To get all useful data from budget
    return db.query(structure.BudgetAnalytics).filter(
        structure.BudgetAnalytics.user_id_user == user_id
    ).all()

def delete_user_budget(db: sqlalchemy.orm.Session, budget_id: int, user_id: int):
    budget = db.query(structure.Budget).filter(
