...
def sync_bank_transactions(
    db: sqlalchemy.orm.Session, request: enable_banking_dto.SyncRequest, user_id: int
) -> dict:
    account = (
...
def list_aspsps(country: str = "PL") -> dict:
...
def generate_auth_url(request: enable_banking_dto.AuthUrlRequest) -> dict:
    token = get_bank_token()
...
def handle_bank_callback(
    ...
    if not bank_accounts_data:
        logger.warning("No accounts returned in session for user %s", user_id)
