                ...
                <div className="flex flex-1 flex-col p-4 lg:p-6 gap-6">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">Manage your spending limits for each month</p>
                        </div>
                        <AddBudgetDialog token={token} onBudgetAdded={() => setRefreshTrigger(p => p + 1)} />
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {budgets.map((budget) => {
                                ...
                                const percentage = budget.percent_used !== undefined ? budget.percent_used : Math.min((spent / limit) * 100, 100);
                                const categoryName = budget.category_name || `Category #${budget.categories_id_category}`;

                                return (
                                    <div key={budget.id_budget} className="rounded-xl border bg-card text-card-foreground shadow-sm p-5 flex flex-col gap-3">
